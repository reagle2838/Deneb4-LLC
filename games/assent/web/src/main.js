...
class App {
  ...
  loop() {
    const tick = () => {
      ...
      if (this.mode === 'select') {
        ...
        this.world.frame();
        if (this.mode === 'game') this.positionLabels();
      }
    };
    tick();
  }
  ...
  buildHud() {
      ...
      top: h(`
        <div class="hud-top">
          ...
          <div class="hud-buttons">
            <button class="btn ghost" data-act="codex">Codex</button>
            <button class="btn ghost" data-act="menu">Menu</button>
            <button class="btn primary end-epoch" data-act="end">End Epoch ▸</button>
          </div>
        </div>`),
    ...
    ui.append(this.hud.top, this.hud.standings, this.hud.log, this.hud.panel);
  ...
  renderPanel() {
    ...
    const actions = G.actionList(s, me, p);
  ...
  actionButton(a) {
    ...
    return `<button class="action ${a.unique ? 'unique' : ''}" data-id="${a.id}" ${a.enabled ? '' : 'disabled'} title="${esc(a.desc)}">
      <span class="name">${esc(a.name)}</span><span class="cost">${a.cost} ⬡</span>
  ...
  act(actionId) {
    if (this.busy) return;
    const r = G.perform(this.state, this.state.player, actionId, this.selected);
    if (!r.ok) { this.toast(r.message); return; }
    this.world.playAction(r.fx);
    if (r.fx.rival) this.world.playRipple(this.selected, new THREE.Color(KINDS[r.fx.rival].color), 1);
  ...
  openIntro() {
    ...
    const w = this.modal(`
      ...
      <p class="hint" style="font-size:14px">Select a polity on the globe. <b>Listen</b> to learn what it really values, <b>Reason</b> where your doctrine matches theirs, <b>Offer</b> gifts sparingly, and think carefully before you <b>Whisper</b>, because the Witness is always watching. When your Compute runs low, <b>End the Epoch</b>. Keep global Heat under 100, or everyone loses.</p>
    ...
    w.querySelector('[data-act="go"]').onclick = () => w.remove();
  ...
  onKey(e) {
    ...
    if (e.key === 'Escape') {
      const m = ui.querySelector('.modal-wrap');
      if (m && m.querySelector('.codex')) m.remove();
      else if (this.mode === 'game') this.selectPolity(null);
    }
    if (this.mode !== 'game' || ui.querySelector('.modal-wrap')) return;
    if (e.code === 'Space') { e.preventDefault(); this.endEpoch(); }
    if (e.key === 'c' || e.key === 'C') this.openCodex();
  }
}
